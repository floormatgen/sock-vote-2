...
extension VoteCalculationTests {
  ...
  struct PreferentialTests {
    ...
    func test_hasNoResultWithNoVotes() throws {
      let result = try Question.preferentialResult(
        using: EmptyCollection(), options: defaultOptions)
      #expect(result == .noVotes)
    }

    @Test(
      "Throws on invalid vote",
      arguments: [
        .init(
          "Too few options",
          votes: [
    ...
    func test_canFindConcreteWinner(_ testArgument: TestArgument) throws {
      let result = try Question.preferentialResult(
        using: testArgument.votes, options: testArgument.options)
