...
struct QuestionTests {
  ...
  func test_cannotVoteMoreThanOnce(_ style: Question.VotingStyle) throws {
    ...
    func vote(flip: Bool) throws {
      ...
      case .plurality:
        ...
        try question.registerPluralityVote(vote, participantToken: token)
      case .preferential:
        let vote = Question.PreferentialVote(
          selectionOrder: flip ? Array(question.options.reversed()) : question.options)
        try question.registerPreferentialVote(vote, participantToken: token)
      }
    }
  ...
  @Test(
    "Question.VotingStyle LosslessStringConvertible round trip",
    arguments: Question.VotingStyle.allCases)
  func test_questionVotingStyleLosslessStringConvertibleRoundTrip(_ style: Question.VotingStyle)
    throws
  {
  ...
  @Test(
    "[preferential] Question.result is correct when queried",
    arguments: [
      .init(
        "Basic winner",
        votes: [
