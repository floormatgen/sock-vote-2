...
extension RoomHandler {
  ...
  func joinRequests(
    ...
    let output = try await getRoomCodeJoinRequests(
      .init(path: .init(code: code), headers: .init(roomAdminToken: adminToken)))
    let body = try output.ok.body.json.requests
    return body.map {
      JoinRequest(
        name: $0.name, participantToken: $0.participantToken, timestamp: $0.timestamp,
        fields: $0.fields?.additionalProperties ?? [:])
    }
  }
  ...
  func requestRoomJoin(
    ...
    let output = try await postRoomCodeJoin(
      .init(
        path: .init(code: code),
        body: .json(.init(name: name, fields: .init(additionalProperties: fields ?? [:])))))
    let body = try output.ok.body.json
  ...
  func handleJoinRequests(
    ...
    accepted: [String]?, rejected: [String]?, failed: [String]?, status: HTTPResponse.Status
  ) {
    let output = try await postRoomCodeJoinRequests(
      .init(
        path: .init(code: code),
