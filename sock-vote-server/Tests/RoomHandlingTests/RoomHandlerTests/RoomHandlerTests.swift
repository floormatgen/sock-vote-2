...
final class RoomHandlerTests: Sendable {
  ...
  static func createRoomWithResponse(
    ...
    try await roomHandler.postRoomCreate(
      .init(
        body: .json(
          .init(
  ...
  typealias PendingJoinRequest = Operations.GetRoomCodeJoinRequests.Output.Ok.Body.JsonPayload
    .RequestsPayloadPayload
  ...
  static func forceJoinRoom(
    ...
    let participantToken = try #require(
      requests.first {
        $0.name == name && $0.fields?.additionalProperties == fields
      }
    ).participantToken
