  ...
  struct ParticipantTests {
    ...
    func test_timeoutFunctionIsCalledForJoinRequest() async throws {
      ...
      try await confirmation { confirmation in
        ...
        let (participantToken, _) = try #require(
          await room.joinRequests.first {
            $0.value.name == Self.defaultParticipantName && $0.value.fields == Self.defaultFields
          })
    ...
    static var defaultFields: [String: String] {
      Dictionary(
        uniqueKeysWithValues: RoomTests.defaultFields.map {
          ($0, "\($0)")
        })
    }
