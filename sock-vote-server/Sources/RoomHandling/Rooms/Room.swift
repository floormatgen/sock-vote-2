...
public protocol RoomProtocol: Actor {
  ...
  func updateQuestion(
    prompt: String, options: some Collection<String> & Sendable, style: Question.VotingStyle) throws
...
extension RoomProtocol {
  ...
  public nonisolated func validateFields(
  ...
  public nonisolated func validateFieldKeys(
  ...
  @discardableResult
  public func handleJoinRequests(
    _ accept: Bool, forTokens participantTokens: some Collection<String>
...
>: RoomProtocol {
  ...
  internal init(
    ...
    self.connectionManager = ConnectionManager(
      logger: Logger(label: "ConnectionManager(room: \(code))"))
  }
...
extension Room {
  ...
  public nonisolated func verifyAdminToken(_ adminToken: String) -> Bool {
    return self.adminToken == adminToken
  }

  public func handleJoinRequest(_ accept: Bool, forToken participantToken: String)
    -> JoinRequestResult
  {
  ...
  public var hasCurrentQuestion: Bool {
    currentQuestion != nil
  }

  public func hasQuestion(with id: UUID) -> Bool {
    currentQuestion?.id == id
  }

  public var currentQuestionDescription: Question.Description? {
    currentQuestion?.questionDescription
  }

  public func updateQuestion(
    prompt: String, options: some Collection<String> & Sendable, style: Question.VotingStyle
  ) throws {
  ...
  public func removeQuestion() throws -> Bool {
  ...
  public func setCurrentQuestionState(to newState: Question.State) throws {
  ...
  public var currentQuestionState: Question.State? {
    guard let question = self.currentQuestion else { return nil }
    return question.state
  }

  public var currentQuestionResult: Question.Result? {
    get throws {
      guard let question = self.currentQuestion else { return nil }
  ...
  public var currentQuestionVoteCount: Int? {
    guard let question = self.currentQuestion else { return nil }
    return question.voteCount
  }

  public func hasParticipant(withParticipantToken participantToken: String) -> Bool {
    return
      (activeParticipants.contains(participantToken)
      || inactiveParticipants.keys.contains(participantToken))
  }

  public func registerPluralityVote(
    _ vote: Question.PluralityVote,
    forParticipant participantToken: String
  ) throws {
    guard let question = self.currentQuestion else {
  ...
  public func registerPreferentialVote(
    _ vote: Question.PreferentialVote,
    forParticipant participantToken: String
  ) throws {
    guard let question = self.currentQuestion else {
  ...
  public nonisolated func runConnectionManager() async throws {
    try await connectionManager.run()
  }

  public func addParticipantConnection(
    _ participantConnection: ParticipantConnection,
    forParticipantToken participantToken: String
  ) async throws {
    assert(
    ...
    try await connectionManager.registerConnection(
      participantConnection, forParticipantToken: participantToken)
  }
...
extension Room {
  ...
  fileprivate func sendQuestionUpdate(_ question: Question?) throws {
...
extension Room {
  ...
  fileprivate func addJoinRequest(_ joinRequest: JoinRequest, participantToken: String) {
    joinRequests[participantToken] = joinRequest
    joinRequestTimeouts[participantToken] = Task(
      name: "Pending Join Request for \"\(joinRequest.name)\""
    ) {
  ...
  fileprivate func makeInactive(participantToken: String) {
